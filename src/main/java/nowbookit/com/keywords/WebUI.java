...
//            return true;
    ...
    public static void openAdminLoginPage() {
        ...
        DriverManager.getDriver().get(FrameworkConstants.ADMIN_LOGIN_PAGE_URL);
        waitForPageLoaded();

        LogUtils.info("Open Admin Login Page URL: " + FrameworkConstants.ADMIN_LOGIN_PAGE_URL);

        if (ExtentTestManager.getExtentTest() != null) {
            ExtentReportManager.pass("Open Admin URL: " + FrameworkConstants.ADMIN_LOGIN_PAGE_URL);
        }
        addScreenshotToReport(Thread.currentThread().getStackTrace()[1].getMethodName() + "_" + DateUtils.getCurrentDateTime());
    }
